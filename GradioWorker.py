...
class GradioInterface:
    ...
    def run_clip_generator(self, *text_prompts):
        ...
        try:
            ...
            set_logger(LogType.LT_EXCEPTION, str(e))
            self.set_status_text(f"에러가 발생했습니다. 프로그램을 다시 실행시켜주세요:  {str(e)}")
    ...
    def generate_merged_video(self, *args):
        ...
        try:
            ...
            self.set_status_text(f"비디오가 만들어졌습니다. 폴더 저장 위치 : \\\\192.168.1.173\\4.result\\final\\{datetime.now().strftime('%Y%m%d-%H%M%S')}.mp4')")
    ...
    def select_que_df(self, evt: gr.SelectData, que_table):
        try:
            if que_table.empty or not evt.index or len(evt.index) == 0:
                set_logger(LogType.LT_WARNING, "선택된 행이 없거나 테이블이 비어있습니다.")
                return [[], []]
    ...
    def launch(self):
        try:
            ...
            self.demo.queue().launch(allowed_paths=[self.args.output_path], prevent_thread_lock=True)
        except Exception as e:
            set_logger(LogType.LT_EXCEPTION, str(e))
    ...
    def close(self):
        try:
            self.demo.close()
            set_logger(LogType.LT_INFO, 'Close GradioWorker')
